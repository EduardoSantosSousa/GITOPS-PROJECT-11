...
class DataProcessing:
    ...
    def preprocess(self):
        try:
            ...
            logger.info("All base data preprocessing done....")
    ...
    def split_and_scale_and_save(self):
        try:
            self.features = ['Operation_Mode', 'Temperature_C', 'Vibration_Hz', 'Power_Consumption_kW', 'Network_Latency_ms', 'Packet_Loss_%',
            'Quality_Control_Defect_Rate_%', 'Production_Speed_units_per_hr','Predictive_Maintenance_Score', 'Error_Rate_%', 
            'Year', 'Month', 'Day', 'Hour']  
            ...
            logger.info("All things saved sucessfuly for dataprocessing.....")

        except Exception as e:
            logger.error(f"Error while split scale and save data {e}")
            raise CustomException("Failed to scale and save data", e)
